...
func (p Planner) singleSport(sport profile.Sport, userPrompt string) {
	actor := openai.NewActor(p.client, openai.O3Mini, p.systemPrompt(), nil)
	actorOutputPlan := actorOutputPlan(p.client, openai.O3Mini, systemPromptSummarize)
...
func (p Planner) MultiStep(interactive bool, numDays int) {
	...
	actor := openai.NewActor(p.client, openai.O3Mini, systemPrompt, nil)
...
func (p Planner) SingleStep(interactive bool, numDays int) {
	...
	actor := actorOutputPlan(p.client, openai.O3Mini, systemPrompt)
	...
	if interactive {
		actorLoop := openai.NewActor(p.client, openai.O3Mini, systemPrompt, nil)
